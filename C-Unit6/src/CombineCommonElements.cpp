...
struct ObjectNode * createIntObjectNode(int *number, int(*equals)(void *, void *)) {
	struct ObjectNode *node1 = (struct ObjectNode *)malloc(sizeof(struct ObjectNode));
	node1->value = (void *)number;
	node1->equals = equals;
...
struct ObjectNode ** combine_common_elements(struct ObjectNode ** objectNodes1, struct ObjectNode ** objectNodes2, int len1, int len2, int *commonElementsLength) {
	...
	int val1,val2,k=0,res;
	struct ObjectNode ** result = (struct ObjectNode **)malloc(sizeof(struct ObjectNode *) * 100);
	for(int i = 0; i < len1; i++)
	{
		val1 = *(int*)(objectNodes1[i]->value);
		for (int j = 0; j < len2; j++)
		{
		val2 = *(int*)(objectNodes2[j]->value);
			if(val1 == val2)
			{
				int f = 0;
				for (int x = 0; x <k; x++)
				{
					 res = *(int*)(result[x]->value);
					 if ( res == val1)
					{
						f = 1;
					}
...
void test_combine_common_elements() {
	...
	struct ObjectNode * intv = intvalues1[0];
	printf("intv[0]=%d\n",intv);
	for (int i = 0; i<len2; i++){
		intvalues2[i] = createIntObjectNode(&arr2[i], &equalsInt1);
	}

	struct ObjectNode ** intvalues3= combine_common_elements(intvalues1, intvalues2, len1, len2, commonelements);
	printf("hi3=%d",(intvalues3[0]));
}

...
void task6() {
	...
	char c = 'a';
	char  *d = "voidpointers";
	typeCastToAllDataTypes(&a, &b, &c, &d[0]);//.You need to find out
...
void task7() {
	void * temp;
     temp= getFloatAsVoidStar();
	printf("%f\n",*((float*)temp) );
	temp = getIntAsVoidStar();
	printf("%d\n", *((int*)temp));
	temp = getStringAsVoidStar();
	printf("%s", ((char*)temp));
	// getIntAsVoidStar and getStringAsVoidStar and print their values.
...
void ** getTwoGreater(void *addr1, void *addr2, void *addr3, int(*compare)(void *, void *)) {
	...
	int res=compare(addr1, addr2);
	if (res == 1)
	{
       int res1=compare(addr2, addr3);
	   if (res1 == 1)
	   {
		   result[0] = addr1;
		   result[1] = addr2;
